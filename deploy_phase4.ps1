Write-Host "SENTRi Phase 4 Deploy Starting..." -ForegroundColor Cyan

# Create directories if they don't exist
New-Item -ItemType Directory -Force -Path "src\store" | Out-Null
New-Item -ItemType Directory -Force -Path "src\pages\command" | Out-Null
New-Item -ItemType Directory -Force -Path "src\pages\gate" | Out-Null
New-Item -ItemType Directory -Force -Path "src\pages\admin" | Out-Null

Write-Host "Writing store/index.js..." -ForegroundColor Yellow
Set-Content -Path "src\store\index.js" -Encoding UTF8 -Value @'
...
Write-Host "Writing CommandApp.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\command\CommandApp.jsx" -Encoding UTF8 -Value @'
        ...
        <div className="header-left">
          <div className="sentri-logo">
            ...
            <div>
              ...
              <div className="officer-info">{officer?.rank} {officer?.name} · {officer?.role === 'command' ? 'Intelligence' : officer?.role}</div>
            </div>
          </div>
        </div>
...
Write-Host "Writing GatesTab.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\command\GatesTab.jsx" -Encoding UTF8 -Value @'
...
export default function GatesTab() {
  ...
  async function createGate() {
    ...
    setCreating(true)
    const { error: err } = await supabase.from('gates').insert({ tenant_id: tenant.id, name: form.name.trim(), slug, location: form.location.trim() || null, is_active: true })
    setCreating(false)
    ...
    <div className="gates-tab">
      <div className="tab-header">
        <div>
          ...
          <p className="tab-sub">Each gate has a unique URL. Send it to the guard device — open in Chrome and tap "Add to home screen."</p>
        </div>
        <button className="btn-primary" onClick={() => { setShowForm(!showForm); setError('') }}>{showForm ? 'Cancel' : '+ Add Gate'}</button>
      </div>
      {showForm && (
      ...
      {loading ? <div className="loading-state">Loading gates...</div> : gates.length === 0 ? (
        <div className="empty-state"><div className="empty-icon">🚧</div><p>No gates configured yet. Add your first gate above.</p></div>
      ) : (
...
Write-Host "Writing IncidentsTab.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\command\IncidentsTab.jsx" -Encoding UTF8 -Value @'
import { useState, useEffect } from 'react'
import { supabase } from '../../lib/supabase'
import { useAuthStore } from '../../store'

const TYPE_LABELS = { unauthorized_access: 'Unauthorized Access', suspicious_vehicle: 'Suspicious Vehicle', suspicious_person: 'Suspicious Person', altercation: 'Altercation', medical_emergency: 'Medical Emergency', equipment_issue: 'Equipment Issue', perimeter_breach: 'Perimeter Breach', contraband_detected: 'Contraband Detected', other: 'Other' }
...
export default function IncidentsTab() {
  ...
  async function fetchIncidents() {
    setLoading(true)
    let query = supabase.from('incidents').select('*, gates(name), officers!incidents_officer_id_fkey(name, rank)').eq('tenant_id', tenant.id).order('created_at', { ascending: false })
  ...
  async function updateStatus(id, status) {
    await supabase.from('incidents').update({ status, acknowledged_by: status === 'acknowledged' ? officer.id : undefined, acknowledged_at: status === 'acknowledged' ? new Date().toISOString() : undefined, resolved_by: status === 'resolved' ? officer.id : undefined, resolved_at: status === 'resolved' ? new Date().toISOString() : undefined }).eq('id', id)
    fetchIncidents()
  }
    ...
    <div className="incidents-tab">
      <div className="tab-header">
        <div>
          ...
          <p className="tab-sub">All reported incidents from gate officers. Critical incidents trigger immediate email alerts.</p>
        </div>
      </div>
      ...
      {loading ? <div className="loading-state">Loading incidents...</div> : incidents.length === 0 ? (
        <div className="empty-state"><div className="empty-icon">✅</div><p>No {filter !== 'all' ? filter : ''} incidents.</p></div>
      ) : (
        <div className="incidents-list">
          {incidents.map(inc => (
            <div key={inc.id} className={`incident-card severity-${inc.severity}`}>
              ...
              {inc.location && <p className="incident-location">Location: {inc.location}</p>}
              {inc.status === 'open' && (
...
Write-Host "Writing ReportTab.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\command\ReportTab.jsx" -Encoding UTF8 -Value @'
import { useState, useEffect } from 'react'
import { supabase } from '../../lib/supabase'
import { useAuthStore } from '../../store'
...
export default function ReportTab() {
  ...
  async function fetchReport() {
    ...
    movements.forEach(m => {
      ...
      byDay[day].total++
      if (m.type === 'vehicle') byDay[day].vehicles++
      else byDay[day].pedestrians++
    ...
    movements.forEach(m => {
      ...
      byGate[name].total++
      if (m.type === 'vehicle') byGate[name].vehicles++
      else byGate[name].pedestrians++
    })
    const durMovements = movements.filter(m => m.duration_minutes)
    const avgDuration = durMovements.length > 0 ? Math.round(durMovements.reduce((s, m) => s + m.duration_minutes, 0) / durMovements.length) : null
      ...
      {loading ? <div className="loading-state">Generating report...</div> : !data ? null : (
        <div className="report-content">
          <div className="stats-grid">
            <div className="stat-card"><div className="stat-label">Total Movements</div><div className="stat-value">{data.total}</div></div>
            ...
            <div className="stat-card"><div className="stat-label">Flag Hits</div><div className="stat-value stat-red">{data.flags}</div></div>
            ...
            <div className="stat-card"><div className="stat-label">Critical Incidents</div><div className="stat-value stat-red">{data.criticalIncidents}</div></div>
...
Write-Host "Writing GateApp.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\gate\GateApp.jsx" -Encoding UTF8 -Value @'
import { useEffect, useState } from 'react'
...
export default function GateApp() {
  ...
  if (loading) return <div className="gate-loading"><div className="gate-loading-logo">⬡</div><p>SENTRi</p></div>
  if (error) return <div className="gate-error"><div className="gate-error-icon">⚠</div><h2>Access Denied</h2><p>{error}</p></div>
...
Write-Host "Writing GateLogPage.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\gate\GateLogPage.jsx" -Encoding UTF8 -Value @'
import { useState, useEffect } from 'react'
import { supabase } from '../../lib/supabase'
import { useGuardStore } from '../../store'
...
export default function GateLogPage({ onBack }) {
  ...
  async function fetchMovements() {
    ...
    const { data } = await supabase.from('movements').select('*, entry_officer:officers!movements_entry_officer_id_fkey(name, rank), exit_officer:officers!movements_exit_officer_id_fkey(name, rank)').eq('tenant_id', tenant.id).eq('gate_id', gate.id).gte('entry_time', since).order('entry_time', { ascending: false })
    setMovements(data || [])
    setLoading(false)
  }

  const filtered = filter === 'all' ? movements : filter === 'inside' ? movements.filter(m => !m.exit_time) : filter === 'flagged' ? movements.filter(m => m.flag_triggered) : movements.filter(m => m.type === filter)
  ...
  return (
    <div className="gate-log-page">
      ...
      {loading ? <div className="loading-state">Loading log...</div> : filtered.length === 0 ? <div className="empty-state">No entries for this period.</div> : (
        <div className="log-list">
          {filtered.map(m => (
            <div key={m.id} className={`log-entry ${m.flag_triggered ? 'flagged' : ''} ${!m.exit_time ? 'inside' : ''}`}>
              <div className="log-entry-header">
                <div className="log-entry-id">{m.type === 'vehicle' ? '🚗' : '🚶'} {m.plate_number || m.visitor_name || 'Unknown'}{m.flag_triggered && <span className="flag-indicator"> ⚠ FLAGGED</span>}</div>
                <div className="log-entry-time">{new Date(m.entry_time).toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit' })}{period === 'week' && <span className="log-date"> · {new Date(m.entry_time).toLocaleDateString('en-NG', { weekday: 'short', day: 'numeric' })}</span>}</div>
              </div>
              <div className="log-entry-details"><span>📍 {m.destination}</span><span> · {m.purpose}</span>{m.occupants > 1 && <span> · {m.occupants} occupants</span>}</div>
              <div className="log-entry-footer">
                ...
                {m.entry_officer?.name && <span className="log-officer">By {m.entry_officer.rank} {m.entry_officer.name}</span>}
              </div>
            </div>
          ))}
        </div>
...
Write-Host "Writing ReportIncidentPage.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\gate\ReportIncidentPage.jsx" -Encoding UTF8 -Value @'
...
export default function ReportIncidentPage({ onBack }) {
  ...
  async function submitIncident() {
    ...
    const { error: err } = await supabase.from('incidents').insert({ tenant_id: tenant.id, gate_id: gate.id, officer_id: officerData?.id || null, type: form.type, severity: form.severity, description: form.description.trim(), location: form.location.trim() || null, status: 'open' })
    setSubmitting(false)
    if (err) { setError(err.message); return }
    setSubmitted(true)
  }
  ...
  if (submitted) return (
    <div className="incident-submitted">
      ...
      {form.severity === 'critical' && <div className="critical-notice">CRITICAL incident — Command has been alerted immediately.</div>}
      <button className="btn-primary full-width" onClick={onBack}>Back to Gate</button>
    </div>
  )

  return (
    <div className="report-incident-page">
      <div className="page-header"><button className="btn-back" onClick={onBack}>← Back</button><h2>Report Incident</h2><p>{gate?.name} · {guard?.name}</p></div>
      {error && <div className="error-msg">{error}</div>}
      <div className="form-section">
        <label>Incident Type *</label>
        <div className="type-grid">
          {INCIDENT_TYPES.map(t => <button key={t.value} className={`type-btn ${form.type === t.value ? 'selected' : ''}`} onClick={() => setForm(f => ({ ...f, type: t.value }))}>{t.label}</button>)}
        </div>
      </div>
      <div className="form-section">
        <label>Severity *</label>
        <div className="severity-grid">
          {SEVERITIES.map(s => <button key={s.value} className={`severity-btn ${form.severity === s.value ? 'selected' : ''}`} style={form.severity === s.value ? { borderColor: s.color, background: s.color + '15' } : {}} onClick={() => setForm(f => ({ ...f, severity: s.value }))}><span className="severity-label" style={{ color: s.color }}>{s.label}</span><span className="severity-desc">{s.desc}</span></button>)}
        </div>
      </div>
      <div className="form-section"><label>Description *</label><textarea placeholder="Describe what happened in detail..." rows={4} value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} /></div>
      <div className="form-section"><label>Specific Location (optional)</label><input placeholder="e.g. Gate 2 entrance, North fence line" value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} /></div>
      <button className={`btn-submit-incident ${form.severity === 'critical' ? 'critical' : ''}`} onClick={submitIncident} disabled={submitting}>{submitting ? 'Submitting...' : form.severity === 'critical' ? '🚨 Submit Critical Incident' : 'Submit Incident Report'}</button>
    </div>
  )
}
'@

Write-Host "Writing AdminApp.jsx..." -ForegroundColor Yellow
Set-Content -Path "src\pages\admin\AdminApp.jsx" -Encoding UTF8 -Value @'
import { useState, useEffect } from 'react'
import { supabase } from '../../lib/supabase'
...
function OverviewTab() {
  ...
  useEffect(() => {
    async function load() {
      const [tenantsRes, movementsRes, incidentsRes, officersRes, gatesRes] = await Promise.all([
      ...
      setStats({
        ...
        activeTenants: (tenantsRes.data||[]).filter(t => t.is_active).length,
        totalMovements: (movementsRes.data||[]).length,
        insideNow: (movementsRes.data||[]).filter(m => !m.exit_time).length,
        totalIncidents: (incidentsRes.data||[]).length,
        openIncidents: (incidentsRes.data||[]).filter(i => i.status === 'open').length,
        criticalIncidents: (incidentsRes.data||[]).filter(i => i.severity === 'critical' && i.status === 'open').length,
        totalOfficers: officersRes.count || 0,
        activeGates: (gatesRes.data||[]).filter(g => g.is_active).length,
  ...
  if (loading) return <div className="loading-state">Loading platform overview...</div>
  return (
    <div className="overview-tab">
      ...
      <div className="stats-grid">
        ...
        {stats.criticalIncidents > 0 && <div className="stat-card critical-card"><div className="stat-label">CRITICAL Open</div><div className="stat-value stat-red">{stats.criticalIncidents}</div></div>}
      </div>
      <h3>Tenants</h3>
...
function TenantsTab() {
  ...
  function slugify(name) { return name.toLowerCase().trim().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') }
  async function saveTenant() {
    ...
    setForm({ name: '', slug: '', sector: '', branch: '', city: '', state: '', country: 'Nigeria', contact_name: '', contact_email: '', contact_phone: '', report_emails: '', report_frequency: 'weekly' })
    fetchTenants()
  }
  async function toggleTenant(t) { await supabase.from('tenants').update({ is_active: !t.is_active }).eq('id', t.id); fetchTenants() }
      ...
      {showForm && (
        <div className="card form-card">
          ...
          <div className="form-grid">
            <div className="form-group"><label>Installation Name *</label><input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Ikeja Cantonment" /></div>
            <div className="form-group"><label>URL Slug</label><input value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} placeholder="auto-generated" /></div>
            ...
            <div className="form-group"><label>Report Emails (comma-separated)</label><input value={form.report_emails} onChange={e => setForm(f => ({ ...f, report_emails: e.target.value }))} /></div>
          ...
          <button className="btn-primary" onClick={saveTenant} disabled={saving}>{saving ? 'Saving...' : selected ? 'Update' : 'Create Tenant'}</button>
        </div>
      )}
        ...
        <div className="tenants-list">
          {tenants.map(t => (
            <div key={t.id} className={`tenant-card ${!t.is_active ? 'inactive' : ''}`}>
              ...
              <div className="tenant-actions"><button className="btn-ghost" onClick={() => editTenant(t)}>Edit</button><button className="btn-ghost" onClick={() => toggleTenant(t)}>{t.is_active ? 'Deactivate' : 'Activate'}</button></div>
            </div>
          ))}
        </div>
      )}
...
function OfficersTab() {
  ...
  async function fetchAll() {
    setLoading(true)
    const [officersRes, tenantsRes] = await Promise.all([
    ...
    setOfficers(officersRes.data || []); setTenants(tenantsRes.data || []); setLoading(false)
  }
  async function toggleOfficer(o) { await supabase.from('officers').update({ is_active: !o.is_active }).eq('id', o.id); fetchAll() }
  const filtered = filterTenant === 'all' ? officers : officers.filter(o => o.tenant_id === filterTenant)
  return (
    <div className="officers-tab">
      <div className="tab-header"><h2>Officers ({filtered.length})</h2><select value={filterTenant} onChange={e => setFilterTenant(e.target.value)} className="tenant-filter"><option value="all">All Tenants</option>{tenants.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
      {loading ? <div className="loading-state">Loading...</div> : (
        <div className="report-table">
          ...
          {filtered.map(o => (
            <div className="table-row" key={o.id}>
              ...
              <span className="action-btns"><button className="btn-xs" onClick={() => toggleOfficer(o)}>{o.is_active ? 'Deactivate' : 'Activate'}</button></span>
            </div>
          ))}
        </div>
      )}
...
function AdminIncidentsTab() {
  ...
  async function fetchIncidents() {
    setLoading(true)
    let query = supabase.from('incidents').select('*, tenants(name), gates(name), officers!incidents_officer_id_fkey(name, rank)').order('created_at', { ascending: false })
    if (filter !== 'all') query = query.eq('status', filter)
    const { data } = await query
...
function SettingsTab() {
  return (
    <div className="settings-tab">
      ...
      <div className="card"><h3>About SENTRi</h3><p>Version: 1.0.0 — Phase 4</p><p>Built by IGATA Technologies</p></div>
      <div className="card"><h3>Email Alerts</h3><p>Incident email notifications scheduled for Phase 6 — Nodemailer + Gmail SMTP.</p></div>
    </div>
  )
}
'@
...
git add -A
git commit -m "Phase 4 — Gates, Incidents, Log, Superadmin, Report periods"
git push origin main

Write-Host ""
Write-Host "✅ Phase 4 deployed successfully!" -ForegroundColor Green
Write-Host "Netlify will auto-deploy in ~30 seconds." -ForegroundColor Cyan
