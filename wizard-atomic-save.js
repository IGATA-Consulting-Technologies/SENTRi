...
console.log('SENTRi — Wizard atomic save: collect all, write once')
...
console.log('✓ OnboardingWizard.jsx — atomic save, all data written in one transaction')

// Git
try {
  execSync('git add src/pages/auth/OnboardingWizard.jsx', { stdio: 'inherit' })
  execSync('git commit -m "Refactor: wizard collects all data first, saves atomically in one write"', { stdio: 'inherit' })
...
console.log('\nDone. Wizard now:')
console.log('- Steps 1-5 collect data in React state only — zero DB writes')
console.log('- Step 5 shows a summary review before saving')
console.log('- One button: Complete setup — writes tenant + officer + all gates in sequence')
console.log('- If anything fails, nothing is saved, clear error shown')
console.log('- Notification email fires after successful save')
console.log('')
console.log('Also run this in Supabase SQL Editor to fix sector constraint:')
console.log("ALTER TABLE tenants DROP CONSTRAINT IF EXISTS tenants_sector_check;")
console.log("ALTER TABLE tenants ADD CONSTRAINT tenants_sector_check")
console.log("CHECK (sector = ANY (ARRAY['military','oil_gas','banking','corporate','government','other']));")
